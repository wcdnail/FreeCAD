#!/bin/bash

pacman -S \
 mingw-w64-clang-x86_64-clang \
 mingw-w64-clang-x86_64-clang-tools-extra \
 mingw-w64-clang-x86_64-gdb \
 mingw-w64-clang-x86_64-qt6-base \
 mingw-w64-clang-x86_64-qt6-3d \
 mingw-w64-clang-x86_64-qt6-datavis3d \
 mingw-w64-clang-x86_64-qt6-multimedia \
 mingw-w64-clang-x86_64-qt6-imageformats \
 mingw-w64-clang-x86_64-qt6-declarative \
 mingw-w64-clang-x86_64-qt6-5compat \
 mingw-w64-clang-x86_64-qt6-svg \
 mingw-w64-clang-x86_64-qt6-tools \
 mingw-w64-clang-x86_64-pyside6 \
 mingw-w64-clang-x86_64-pyside6-tools \
 mingw-w64-clang-x86_64-yaml-cpp \
 mingw-w64-clang-x86_64-xerces-c \
 mingw-w64-clang-x86_64-vtk \
 mingw-w64-clang-x86_64-opencascade \
 mingw-w64-clang-x86_64-boost \
 mingw-w64-clang-x86_64-python-numpy \
 mingw-w64-clang-x86_64-python-pivy \
 mingw-w64-clang-x86_64-python-pip \
 mingw-w64-clang-x86_64-coin \
 mingw-w64-clang-x86_64-eigen3 \
 mingw-w64-clang-x86_64-swig \
 $@
